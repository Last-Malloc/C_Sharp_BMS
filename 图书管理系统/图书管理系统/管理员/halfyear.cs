...
namespace 图书管理系统
{
    ...
    public partial class halfyear : Form
    {
        public int b;
        public halfyear(string key)
        {
            InitializeComponent();
            b = Int32.Parse(key);
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            //声明查询语句
            string sqlquarter = String.Format("select abISBN 'ISBN码', sum(abCount) '统计数量',abName '图书名称' from [activeBook] where abTime <= {0} and abTime >= {1} group by abName, abISBN order by sum(abCount) DESC", b, b - 6);
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            string connstring = "Data Source=.;Initial Catalog=BMS;User ID=sa;Pwd=648371";
            //声明查询语句
            string sqlchart = String.Format("select abISBN 'ISBN码', sum(abCount) '统计数量',abName '图书名称' from [activeBook] where abTime <= {0} and abTime >= {1} group by abName, abISBN", b, b - 6);
